...
import { CreditCardIcon, UserIcon, CalendarIcon, BarChart3Icon } from "lucide-react";
...
export default function UserPage({ params }: PageProps) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const portfoliosData = await api.getPortfolios(id);
  ...
  const handleCreatePortfolio = useCallback(async () => {
    try {
      setError(null);
      const newPortfolio = await api.createPortfolio(id, {
      ...
      setError(isAppError(err) ? err : new Error('Failed to create portfolio'));
    }
