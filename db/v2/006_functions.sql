...
CREATE FUNCTION portfolio.fn_PortfolioMarketValueV2 (
    @PortfolioID INT
...
CREATE FUNCTION portfolio.fn_PortfolioTotalInvestment (
    @PortfolioID INT
) RETURNS DECIMAL(18,2)
AS
BEGIN
...
CREATE FUNCTION portfolio.fn_PortfolioUnrealizedGainLoss (
    @PortfolioID INT
) RETURNS DECIMAL(18,2)
AS
BEGIN
...
CREATE FUNCTION portfolio.fn_PortfolioUnrealizedGainLossPct (
    @PortfolioID INT
...
CREATE FUNCTION portfolio.fn_PortfolioTotalValue (
    @PortfolioID INT
) RETURNS DECIMAL(18,2)
AS
BEGIN
...
CREATE FUNCTION portfolio.fn_HoldingCurrentValue (
    @HoldingID BIGINT
) RETURNS DECIMAL(18,2)
AS
BEGIN
...
CREATE FUNCTION portfolio.fn_HoldingUnrealizedGainLoss (
    @HoldingID BIGINT
) RETURNS DECIMAL(18,2)
AS
BEGIN
...
CREATE FUNCTION portfolio.fn_HoldingGainLossPercentage (
    @HoldingID BIGINT
) RETURNS DECIMAL(10,2)
AS
BEGIN
...
CREATE FUNCTION portfolio.fn_UserNetWorth (
    @UserID UNIQUEIDENTIFIER
) RETURNS DECIMAL(18,2)
AS
BEGIN
    DECLARE @AccountBalance DECIMAL(18,2) = 0;
    DECLARE @PortfolioValue DECIMAL(18,2) = 0;
    ...
    RETURN ISNULL(@AccountBalance, 0) + ISNULL(@PortfolioValue, 0);
END;
GO

-- Calculate days remaining in premium subscription
CREATE FUNCTION portfolio.fn_UserPremiumDaysRemaining (
    @UserID UNIQUEIDENTIFIER
) RETURNS INT
AS
BEGIN
...
CREATE FUNCTION portfolio.fn_UserSubscriptionExpired (
    @UserID UNIQUEIDENTIFIER
) RETURNS BIT
AS
BEGIN
...
CREATE FUNCTION portfolio.fn_AssetPriceChangePercent (
    @AssetID INT,
    @DaysBack INT
) RETURNS DECIMAL(10,2)
AS
...
CREATE FUNCTION portfolio.fn_AssetVolatility (
    @AssetID INT,
    @DaysBack INT = 30
) RETURNS DECIMAL(10,2)
AS
...
CREATE FUNCTION portfolio.fn_CalculateNewAveragePrice (
...
CREATE FUNCTION portfolio.fn_CalculatePartialSaleCostBasis (
...
CREATE FUNCTION portfolio.fn_FormatCurrency (
    @Amount DECIMAL(18,2)
) RETURNS NVARCHAR(50)
AS
BEGIN
...
CREATE FUNCTION portfolio.fn_FormatPercentage (
